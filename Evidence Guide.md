...
For the file extraction which is uesed for getting the images form the user to the application. I used an input tage with the attributes of type image, onchange="loadFile(event)" and accept="image/*". Which only allows file with the file extension of 
